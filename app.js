import { collection, addDoc, getDocs, getDoc, getDocFromServer, setDoc, doc, deleteDoc, writeBatch, query, orderBy, limit } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
function renderAccountInfo(profile){
  ...
  el.innerHTML = `
    <div><strong>Email:</strong> ${escapeHtml(user.email || 'sem email')}</div>
    <div><strong>Nome:</strong> ${escapeHtml(displayName)}</div>
    <div><strong>Saudação:</strong> ${escapeHtml(genderLabel)}</div>
  `;
}
...
async function refreshWelcomeGate(user){
  try{
    ...
    // Lê localStorage imediatamente para mostrar o nome sem delay
    try{
      const lsKey = getProfileStorageKey(user.uid);
      const raw = localStorage.getItem(lsKey);
      if(raw){
        const cached = normalizeProfile(JSON.parse(raw));
        if(cached.name) updateSplashGreeting(cached);
      }
    }catch(e){}
...
async function deleteAttachmentFromFirestore(url){
  try{
    ...
    for(const d of toDelete){
      await deleteDoc(doc(window.db, col.path, d.id));
    }
  ...
  try{
    ...
    if(item && item.url){
      ...
      try{
        ...
        if(st){
          const sref = storageRef(st, item.url);
          deleteObject(sref).catch(e=>console.warn('deleteObject storage', e));
        }
      }catch(e){ console.warn('removeLocalAttachment storage', e); }
    }
...
function createTaskFromAttachment(id){
  try{
    ...
    modal.dataset.taskIndex = '__new_from_attachment__';
    document.getElementById('edit-task-name').value = a.name || '';
    document.getElementById('edit-task-date').value = todayStr();
    document.getElementById('edit-task-time').value = '';
    document.getElementById('edit-task-cat').value = 'outro';
    document.getElementById('edit-task-est').value = '30';
    document.getElementById('edit-task-message').textContent = '';

    const attDiv = document.getElementById('edit-task-attachments');
    if(attDiv){
      attDiv.innerHTML = `<a href="${escapeHtml(a.url)}" target="_blank">Ver anexo: ${escapeHtml(a.name)}</a>`;
      attDiv.dataset.newAttachment = a.url;
    }

    document.getElementById('edit-task-cancel-btn').onclick = ()=>{ modal.classList.add('hidden'); };

    const form = document.getElementById('edit-task-form');
    const prevSubmit = form ? form.onsubmit : null;
    if(form){
      form.onsubmit = function(ev){
        ev.preventDefault();
        try{
          const name = document.getElementById('edit-task-name').value.trim() || a.name;
          const date = document.getElementById('edit-task-date').value || todayStr();
          const time = document.getElementById('edit-task-time').value || null;
          const cat  = document.getElementById('edit-task-cat').value || 'outro';
          const est  = parseInt(document.getElementById('edit-task-est').value||'30',10)||30;
          const t = { name, date, time: time||null, cat, est, attachment: a.url };
          tasks.push(t); saveTasks(); renderCal(); renderTasks();
          removeLocalAttachment(id);
          modal.classList.add('hidden');
        }catch(e){ console.warn('createTaskFromAttachment save', e); }
        form.onsubmit = prevSubmit;
      };
    }
  }catch(e){ console.warn('createTaskFromAttachment', e); }
}
...
function renderAttachmentsPanel(){
  try{
    ...
    list.forEach(a=>{
      ...
      const meta = document.createElement('div'); meta.style.display='flex'; meta.style.flexDirection='column'; meta.innerHTML = `<div style="font-size:13px">${escapeHtml(a.name)}</div><div style="font-size:11px;color:var(--muted)">${Math.round(a.size/1024)} KB • ${new Date(a.ts).toLocaleString()}</div>`;
  ...
  for(let d=1;d<=dim;d++){
    ...
    if(tasksForDay.length){
      ...
      tasksForDay.slice(0,maxShow).forEach(t=>{
        ...
        mt.innerHTML = `<div class="mini-dot" style="background:${c}"></div><div class="mini-text${completedClass}">${t.time? escapeHtml(t.time)+' ' : ''}${escapeHtml(t.name)}</div>`;
        el.appendChild(mt);
      });
...
function saveToFirebaseDebounced(tasksList){
  if(_saveToFirebaseTimer) clearTimeout(_saveToFirebaseTimer);
  _saveToFirebaseTimer = setTimeout(()=>{
    _saveToFirebaseTimer = null;
    saveToFirebase(tasksList).catch(e=>console.error('saveTasks->saveToFirebase (debounced)', e));
  }, 1500);
}
...
function renderAggregatesUI(agg){
  ...
  for(const k of Object.keys(agg.byCategory)){
    const v = agg.byCategory[k]; html += `<div class="agg-item"><strong>${escapeHtml(k)}</strong> — média ${escapeHtml(String(v.avg))}min (n=${escapeHtml(String(v.n))})</div>`;
  }
  html += '<div class="agg-small" style="margin-top:8px">Tarefas semelhantes:</div>';
  const names = Object.keys(agg.byName).slice(0,6);
  for(const n of names){ const v = agg.byName[n]; html += `<div class="agg-item">${escapeHtml(v.sample)} — média ${escapeHtml(String(v.avg))}min (n=${escapeHtml(String(v.n))})</div>`; }
  html += '</div>';
...
function editTask(i){
  ...
  if(t.attachment){
    ...
    a.innerHTML = `<a href="${escapeHtml(t.attachment)}" target="_blank">Ver anexo</a>`;
  ...
  document.getElementById('edit-task-add-attachment').onchange = function(ev){
    ...
    if(file){
      ...
      attDiv.innerHTML = `<a href="${escapeHtml(url)}" target="_blank">${escapeHtml(file.name)}</a>`;
...
function deleteTaskAttachment(task){
  ...
  try{
    urls.forEach(u => {
      deleteAttachmentFromFirestore(u).catch(e=>console.warn('deleteAttachmentFromFirestore in deleteTaskAttachment', e));
    });
  }catch(e){}
  try{
    const st = window.storage || (typeof getStorage === 'function' && window.firebaseApp ? getStorage(window.firebaseApp) : null);
...
${tasksSummary}`;
  ...
  try {
    ...
    if(!resp.ok){
      let errMsg = `Erro HTTP ${resp.status}`;
      if(resp.status === 429) errMsg = 'Limite de requisições atingido. Aguarde alguns segundos e tente novamente.';
      else if(resp.status === 503) errMsg = 'Serviço da IA temporariamente indisponível. Tente novamente em breve.';
    ...
    if(importEl){
      importEl.addEventListener('change', async (ev)=>{
        ...
        try{
          ...
          const uid = window.auth && window.auth.currentUser ? window.auth.currentUser.uid : 'unknown';
...
function openRecipeModal(recipeId){
  ...
  if(bodyEl){
    let html = '';
    if(r.prepTime) html += `<div style="font-size:13px;color:var(--muted);margin-bottom:4px">⏱ ${escapeHtml(r.prepTime)} min de preparo</div>`;
    if(r.ingredients && r.ingredients.length){
      html += '<div class="recipe-section-title">Ingredientes</div>';
      html += '<ul class="recipe-modal-list">' + r.ingredients.map(i=>`<li>${escapeHtml(i)}</li>`).join('') + '</ul>';
    }
    if(r.steps && r.steps.length){
      html += '<div class="recipe-section-title">Modo de Preparo</div>';
      html += '<ol class="recipe-modal-list">' + r.steps.map(s=>`<li>${escapeHtml(s)}</li>`).join('') + '</ol>';
    }
    if(r.notes){
      html += '<div class="recipe-section-title">Observações</div>';
      html += `<div style="font-size:13px;color:var(--text);padding:8px;border-radius:6px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.02)">${escapeHtml(r.notes)}</div>`;
    }
    if(r.tags && r.tags.length){
      html += '<div class="recipe-section-title">Tags</div>';
      html += '<div class="recipe-tags">' + r.tags.map(t=>`<span class="recipe-tag">${escapeHtml(t)}</span>`).join('') + '</div>';
    }
    if(r.history && r.history.length){
      html += '<div class="recipe-section-title">Histórico</div>';
      html += r.history.map(h=>`<div class="recipe-history-item"><strong>${escapeHtml(h.date||'')}</strong>${h.obs? ' — '+escapeHtml(h.obs) : ''}</div>`).join('');
    }
